    ...
    >
      <div className="w-full max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-12 flex items-center justify-between">
        ...
        <a href="#" className="flex items-center gap-3 group">
          <div className="w-10 h-10 rounded-xl bg-azure-500/10 border border-azure-500/30 flex items-center justify-center text-azure-400 group-hover:bg-azure-500/20 group-hover:border-azure-400 transition-all">
          ...
          <div>
            ...
            <span className="block text-[11px] text-emerald-400 font-mono tracking-wider uppercase">
              Azure Data Engineer
            </span>
          </div>
        </a>
        ...
        {/* Desktop Nav Links */}
        <nav className="hidden md:flex items-center gap-1.5 bg-slate-900/70 backdrop-blur-md px-5 py-2 rounded-full border border-slate-800/90 shadow-md">
          {navLinks.map((link) => {
      ...
      {/* Mobile Drawer */}
      {mobileMenuOpen && (
        <div className="md:hidden bg-slate-950/95 backdrop-blur-xl border-b border-slate-800 px-6 pt-4 pb-6 space-y-3">
          {navLinks.map((link) => (
            <a
              key={link.name}
              href={link.href}
